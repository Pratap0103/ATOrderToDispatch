...
const MeenaOutsideHistory = ({ orders, onEditClick }) => {
  ...
  const tableHeaders = [
    ...
    "Meena Outside Status",
    "Outside Chillai Weight",
    "Outside After Meena Polish",
    "Outside Remarks",
    "Planned Date",
    "Done Date",
    "Delay",
      ...
      <tr key={order.id || idx} className="hover:bg-amber-50/30 transition-colors border-b border-gray-100 group">
        <td className="px-2 py-3 sticky left-0 bg-white group-hover:bg-amber-50 z-10 shadow-[1px_0_0_#e5e7eb] text-center whitespace-nowrap w-32 min-w-[128px]">
          <button
            ...
            title="Edit Meena Outside Details"
          >
            <Edit size={12} />
            <span>Edit</span>
          </button>
        ...
        <td className="px-4 py-3 text-center whitespace-nowrap text-xs">
          <span className="px-2 py-0.5 rounded font-bold border bg-emerald-100 text-emerald-800 border-emerald-200">
            {order.meenaOutsideType || order.meenaOutsideStatus || '-'}
          </span>
        </td>
        <td className="px-4 py-3 text-center whitespace-nowrap text-xs font-bold text-gray-900">
          {order.outsideChillaiWeight ? `${order.outsideChillaiWeight} g` : '-'}
        </td>
        <td className="px-4 py-3 text-center whitespace-nowrap text-xs font-bold text-gray-900">
          {order.outsideAfterMeenaPolish ? `${order.outsideAfterMeenaPolish} g` : '-'}
        </td>
        <td className="px-4 py-3 text-center whitespace-nowrap text-xs text-gray-600 truncate max-w-[150px]" title={order.outsideRemarks || '-'}>
          {order.outsideRemarks || '-'}
        </td>
        <td className="px-4 py-3 text-center text-xs text-gray-600 whitespace-nowrap">
          {formatTargetDate(order.plannedDates?.['Meena Outside'], 'Meena Outside')}
        </td>
        <td className="px-4 py-3 text-center text-xs text-gray-600 whitespace-nowrap">
          {formatDateTime(order.meenaOutsideTimestamp)}
        </td>
        <td className="px-4 py-3 text-center whitespace-nowrap">
          {(() => { const d = calculateDelay(order.plannedDates?.['Meena Outside'], order.meenaOutsideTimestamp); return <span className={`px-2 py-0.5 rounded text-[10px] font-bold border ${d.display === '-' ? 'bg-gray-100 text-gray-500 border-gray-200' : d.isDelayed ? 'bg-red-100 text-red-800 border-red-200' : 'bg-emerald-100 text-emerald-800 border-emerald-200'}`}>{d.display}</span>; })()}
        </td>
    ...
    return (
      <div key={order.id || idx} className="bg-white rounded-xl border border-amber-50 shadow-sm p-4 space-y-3 transition-all hover:shadow-md hover:border-amber-100">
        <div className="flex justify-between items-center pb-2 border-b border-slate-50">
          ...
          <span className="px-2 py-0.5 rounded text-[8px] font-bold border bg-emerald-100 text-emerald-800 border-emerald-200">
            {order.meenaOutsideType || order.meenaOutsideStatus || '-'}
          </span>
        </div>
        <div className="grid grid-cols-2 gap-2 text-[11px] bg-slate-50 rounded-lg p-2 border border-slate-100/50">
          ...
          <div>
            ...
            {(() => { const d = calculateDelay(order.plannedDates?.['Meena Outside'], order.meenaOutsideTimestamp); return <span className={`font-bold ${d.isDelayed ? 'text-red-600' : d.display === '-' ? 'text-gray-400' : 'text-emerald-600'}`}>{d.display}</span>; })()}
          </div>
          <div>
          ...
          <div>
            <span className="text-gray-400 block uppercase text-[8px] tracking-tight">Chillai Weight</span>
            <span className="text-gray-700 font-semibold">{order.outsideChillaiWeight ? `${order.outsideChillaiWeight} g` : '-'}</span>
          </div>
          <div>
            <span className="text-gray-400 block uppercase text-[8px] tracking-tight">After Polish</span>
            <span className="text-gray-700 font-semibold">{order.outsideAfterMeenaPolish ? `${order.outsideAfterMeenaPolish} g` : '-'}</span>
          </div>
          <div className="col-span-2">
            <span className="text-gray-400 block uppercase text-[8px] tracking-tight">Remarks</span>
            <span className="text-gray-700">{order.outsideRemarks || '-'}</span>
          </div>
        </div>
        <div className="pt-2 border-t border-slate-100 mt-1">
          ...
          >
            <Edit size={12} /> Edit Meena Outside
          </button>
        </div>
      </div>
    );
...
export default MeenaOutsideHistory;
