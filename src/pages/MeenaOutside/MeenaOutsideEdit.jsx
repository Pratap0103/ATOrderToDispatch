...
const MeenaOutsideEdit = ({ isOpen, onClose, onSave, order }) => {
  ...
  useEffect(() => {
    if (isOpen && order) {
      setFormData({
        meenaOutsideStatus: order.meenaOutsideType || order.meenaOutsideStatus || '',
  ...
  return (
    <ModalForm
      isOpen={isOpen}
      onClose={onClose}
      title={`Edit Meena Outside — Order: ${order.orderNo || order.orderNumber}`}
...
export default MeenaOutsideEdit;
