...
const MeenaSubmit = () => {
  ...
  const getChillaiWeight = (o) => Number(o.inhouseChillaiWeight || o.outsideChillaiWeight) || 0;
  const getMeenaReceived = (o) => Number(o.inhouseAfterMeenaPolish || o.outsideAfterMeenaPolish) || 0;
  const getMeenaWeight = (o) => o.meenaWeight ? Number(o.meenaWeight) : (getChillaiWeight(o) - getMeenaReceived(o));
  const getType = (o) => o.meenaInhouseType || o.meenaOutsideType || o.orderType || '-';
  ...
  const basePendingOrders = useMemo(() => {
    return orders.filter(o => 
      (o.meenaInhouseStatus === 'Complete' || o.meenaOutsideStatus === 'Complete') && 
      !o.meenaSubmitDone
    );
  }, [orders]);

  const baseHistoryOrders = useMemo(() => {
    return orders.filter(o => o.meenaSubmitDone === true);
  }, [orders]);
  ...
  const totals = useMemo(() => {
    let chillai = 0, received = 0, ghatJama = 0, meenaWt = 0;
    filteredOrders.forEach(o => {
      chillai += getChillaiWeight(o);
      received += getMeenaReceived(o);
      ghatJama += getGhatJama(o);
      meenaWt += getMeenaWeight(o);
    });
    return { chillai, received, ghatJama, meenaWt };
  }, [filteredOrders]);
  ...
  const handleBulkSubmit = () => {
    ...
    const updatedOrders = orders.map(o => {
      if (selectedOrders.has(o.id)) {
        return { ...o, meenaSubmitDone: true, meenaSubmitTimestamp: submissionDate };
      }
      return o;
    });
  ...
  const downloadExcel = () => {
    const title = activeTab === 'pending' ? 'MEENA DETAILS' : 'MEENA HISTORY DETAILS';
    const headers = [
      'Timestamp', 'Karigar Name', 'Voucher Number', 'Melting', 'Order Number',
      'Ghat Weight', 'Chillai Weight', 'After Meena Weight Received', 'Meena Weight',
      'Serial Number', 'Remarks', 'Type'
    ];
    ...
    const dataRows = filteredOrders.map(o => {
      const row = [
        ...
        getChillaiWeight(o).toFixed(3),
        getMeenaReceived(o).toFixed(3),
        getMeenaWeight(o).toFixed(3),
        o.serialNumber || '-',
        getRemarks(o),
        getType(o)
      ];
      if (activeTab === 'history') {
        const dateObj = o.meenaSubmitTimestamp ? new Date(o.meenaSubmitTimestamp) : null;
    ...
    const summaryTotals = [
      ['Chillai Weight', totals.chillai.toFixed(3)],
      ['Meena Received', totals.received.toFixed(3)],
      ['Ghat Jama', totals.ghatJama.toFixed(3)],
      ['Meena Weight', totals.meenaWt.toFixed(3)]
    ];
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Meena Data");
    const timestamp = new Date().toISOString().split('T')[0];
    XLSX.writeFile(wb, `Meena_${activeTab === 'pending' ? 'Details' : 'History'}_${timestamp}.xlsx`);
  };
  ...
  const tableHeaders = [
    ...
    { label: 'Chillai Weight', className: 'text-center whitespace-nowrap' },
    { label: 'After Meena Weight Received', className: 'text-center whitespace-nowrap' },
    { label: 'Meena Weight', className: 'text-center whitespace-nowrap font-bold text-amber-700' },
    { label: 'Serial Number', className: 'text-center whitespace-nowrap' },
    { label: 'Remarks', className: 'text-center whitespace-nowrap' },
  ...
  const renderRow = (order, idx) => (
    <tr key={order.id || idx} className="hover:bg-amber-50/50 transition-colors border-b border-gray-100">
      ...
      <td className="px-4 py-3 text-center text-xs whitespace-nowrap">{getChillaiWeight(order).toFixed(3)}</td>
      <td className="px-4 py-3 text-center text-xs whitespace-nowrap">{getMeenaReceived(order).toFixed(3)}</td>
      <td className="px-4 py-3 text-center text-xs font-bold text-amber-700 whitespace-nowrap bg-amber-50/30">{getMeenaWeight(order).toFixed(3)}</td>
      <td className="px-4 py-3 text-center text-xs whitespace-nowrap">{order.serialNumber || '-'}</td>
      <td className="px-4 py-3 text-center text-xs text-gray-500 max-w-[150px] truncate" title={getRemarks(order)}>{getRemarks(order)}</td>
      ...
      {activeTab === 'history' && (
        <td className="px-4 py-3 text-center text-xs font-semibold whitespace-nowrap">
          {order.meenaSubmitTimestamp ? new Date(order.meenaSubmitTimestamp).toLocaleDateString('en-GB') : '-'}
        </td>
      )}
    </tr>
  );
  ...
  const renderCard = (order) => (
    <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm space-y-3">
      ...
      <div className="grid grid-cols-2 gap-2 text-[11px] bg-slate-50 p-2 rounded-lg border border-slate-100/50">
        ...
        <div className="text-center"><span className="text-gray-400 block uppercase text-[8px] tracking-tight">Ghat Jama</span><span className="font-semibold text-gray-700">{getGhatJama(order).toFixed(3)}</span></div>
        <div className="text-center"><span className="text-gray-400 block uppercase text-[8px] tracking-tight">Chillai Wt</span><span className="font-semibold text-gray-700">{getChillaiWeight(order).toFixed(3)}</span></div>
        <div className="text-center"><span className="text-gray-400 block uppercase text-[8px] tracking-tight">Received Wt</span><span className="font-semibold text-gray-700">{getMeenaReceived(order).toFixed(3)}</span></div>
        <div className="col-span-2 text-center bg-amber-100 p-1.5 rounded font-bold text-amber-800 border border-amber-200 mt-1">
          Meena Wt: {getMeenaWeight(order).toFixed(3)}
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="p-0 sm:p-2 md:p-6 space-y-2 md:space-y-6 flex flex-col h-full min-h-0">
      ...
      {/* Totals Summary Row */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-4 px-2 sm:px-0">
        {[
          { label: 'Chillai Wt', value: totals.chillai },
          { label: 'Received Wt', value: totals.received },
          { label: 'Ghat Jama Wt', value: totals.ghatJama },
          { label: 'Meena Wt', value: totals.meenaWt },
        ].map(tot => (
          <div key={tot.label} className="bg-white border border-gray-200 rounded-xl p-3 flex flex-col items-center justify-center shadow-sm">
            ...
            <span className="text-sm md:text-lg font-black text-amber-600 mt-1">{tot.value.toFixed(3)}</span>
          </div>
        ))}
      </div>
