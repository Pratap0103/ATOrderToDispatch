...
const EPolishHistory = ({ orders, onEditClick }) => {
      ...
      <div key={order.id || idx} className="bg-white rounded-xl border border-amber-50 shadow-sm p-4 space-y-3 transition-all hover:shadow-md hover:border-amber-100">
        ...
        <div className="grid grid-cols-2 gap-2 text-[11px] bg-slate-50 rounded-lg p-2 border border-slate-100/50">
          ...
          <div>
            ...
            {(() => { const d = calculateDelay(order.plannedDates?.['EPolish'], order.ePolishTimestamp); return <span className={`font-bold ${d.isDelayed ? 'text-red-600' : d.display === '-' ? 'text-gray-400' : 'text-emerald-600'}`}>{d.display}</span>; })()}
          </div>
          <div>
          ...
          <div>
            <span className="text-gray-400 block uppercase text-[8px] tracking-tight">E-Polish Weight</span>
            <span className="text-gray-700 font-semibold">{order.ePolishWeight ? `${order.ePolishWeight} g` : '-'}</span>
          </div>
          <div>
            <span className="text-gray-400 block uppercase text-[8px] tracking-tight">E-Polish Loss</span>
            <span className="text-gray-700 font-semibold">{order.ePolishLoss ? `${order.ePolishLoss} g` : '-'}</span>
          </div>
        </div>
