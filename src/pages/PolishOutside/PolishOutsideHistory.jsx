...
const PolishOutsideHistory = ({ orders, onEditClick }) => {
  ...
  const tableHeaders = [
    ...
    "Outside After Polish Weight",
    "Outside Meena Polish Loss",
    "Outside Remarks",
    "Planned Date",
    "Done Date",
    "Delay",
      ...
      <tr key={order.id || idx} className="hover:bg-amber-50/30 transition-colors border-b border-gray-100 group">
        <td className="px-2 py-3 sticky left-0 bg-white group-hover:bg-amber-50 z-10 shadow-[1px_0_0_#e5e7eb] text-center whitespace-nowrap w-32 min-w-[128px]">
          <button
            ...
            title="Edit Polish Outside Details"
          >
            <Edit size={12} />
            <span>Edit</span>
          </button>
        ...
        <td className="px-4 py-3 text-center whitespace-nowrap text-xs font-bold text-gray-900">
          {order.outsideAfterPolishWeight ? `${order.outsideAfterPolishWeight} g` : '-'}
        </td>
        <td className="px-4 py-3 text-center whitespace-nowrap text-xs font-bold text-gray-900">
          {order.outsideMeenaPolishLoss ? `${order.outsideMeenaPolishLoss} g` : '-'}
        </td>
        ...
        <td className="px-4 py-3 text-center text-xs text-gray-600 whitespace-nowrap">
          {formatTargetDate(order.plannedDates?.['Polish Outside'], 'Polish Outside')}
        </td>
        <td className="px-4 py-3 text-center text-xs text-gray-600 whitespace-nowrap">
      ...
      <div key={order.id || idx} className="bg-white rounded-xl border border-amber-50 shadow-sm p-4 space-y-3 transition-all hover:shadow-md hover:border-amber-100">
        ...
        <div className="grid grid-cols-2 gap-2 text-[11px] bg-slate-50 rounded-lg p-2 border border-slate-100/50">
          ...
          <div>
            ...
            <span className="text-gray-700 font-semibold">{order.outsideAfterPolishWeight ? `${order.outsideAfterPolishWeight} g` : '-'}</span>
          </div>
          <div>
            <span className="text-gray-400 block uppercase text-[8px] tracking-tight">Meena Polish Loss</span>
            <span className="text-gray-700 font-semibold">{order.outsideMeenaPolishLoss ? `${order.outsideMeenaPolishLoss} g` : '-'}</span>
          </div>
        ...
        <div className="pt-2 border-t border-slate-100 mt-1">
          ...
          >
            <Edit size={12} /> Edit Polish Outside
          </button>
        </div>
      </div>
    );
...
export default PolishOutsideHistory;
