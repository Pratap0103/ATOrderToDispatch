...
import PolishOutsidePending from './PolishOutsidePending';
...
const PolishOutside = () => {
  ...
  const baseHistoryOrders = useMemo(() => {
    return orders.filter(o => 
      o.polishOutsideStatus && 
      o.polishOutsideStatus !== 'Pending' && 
      o.polishOutsideStatus !== ''
    );
  }, [orders]);
  ...
  // Split orders based on Polish Outside status
  // 1. Pending: Ghat Jama completed as "Polish Outside" OR (completed as "Meena Inhouse"/"Meena Outside" and routed to "Polish Outside") and Polish status is NOT complete/processed
  

  // 2. History: processed Polish Outside orders (has polishOutsideStatus value)
