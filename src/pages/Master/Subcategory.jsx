...
import { Plus, Search, RotateCcw, Layers, Trash2, Edit2, ListTree, Filter } from 'lucide-react';
...
export default function Subcategory({
  ...
  const [localFilterSubcategories, setLocalFilterSubcategories] = useState([]);
  const [currentPage, setCurrentPage] = useState(1);
  const [itemsPerPage, setItemsPerPage] = useState(50);

  const effectiveSearch = isEmbedded ? (externalSearch || '') : localSearch;
  const filterSubcategories = filterValue !== undefined ? filterValue : localFilterSubcategories;
  const setFilterSubcategories = onFilterChange || setLocalFilterSubcategories;
  const subcategoryOptions = useMemo(() => window.generateFilterOptions ? window.generateFilterOptions(subcategories, 'subcategory') : [], [subcategories]);
  ...
  const filtered = useMemo(() => subcategories.filter(c => {
    if (filterSubcategories.length > 0 && !filterSubcategories.includes(c.subcategory)) return false;
    if (effectiveSearch) {
      const q = effectiveSearch.toLowerCase();
      return c.subcategory.toLowerCase().includes(q);
    }
    return true;
  }), [subcategories, effectiveSearch, filterSubcategories]);
