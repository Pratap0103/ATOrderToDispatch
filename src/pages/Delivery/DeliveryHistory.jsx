...
const DeliveryHistory = ({ orders, onEditClick }) => {
  ...
  const tableHeaders = [
    ...
    { label: 'Delivery Status', className: 'text-center' },
  ...
  const renderRow = (order, idx) => {
    ...
    const delStat = order.deliveryStatus || order.status15;
    return (
    ...
    return (
      <div key={order.id || idx} className="bg-white rounded-xl border border-amber-50 shadow-sm p-4 space-y-3 transition-all hover:shadow-md hover:border-amber-100">
        ...
        <div className="grid grid-cols-2 gap-2 text-[11px] bg-slate-50 rounded-lg p-2 border border-slate-100/50 text-center">
          ...
          <div>
            <span className="text-gray-400 block uppercase text-[8px] tracking-tight">Delivery Status</span>
            {delStat ? (
              <span className={`px-1.5 py-0.5 rounded text-[8px] font-bold border inline-block ${getDeliveryStatusColor(delStat)}`}>{delStat}</span>
            ) : <span className="text-gray-400">-</span>}
          </div>
          <div>
          ...
          <div>
            ...
            {(() => { const d = calculateDelay(order.plannedDates?.['Delivery'], order.deliveryTimestamp); return <span className={`font-bold ${d.isDelayed ? 'text-red-600' : d.display === '-' ? 'text-gray-400' : 'text-emerald-600'}`}>{d.display}</span>; })()}
          </div>
        </div>
        {order.deliveryRemarks && (
          <div className="text-[10px] text-gray-500 border-t border-slate-100 pt-1.5 truncate text-center">
            <strong>Remarks: </strong>{order.deliveryRemarks}
          </div>
        )}
