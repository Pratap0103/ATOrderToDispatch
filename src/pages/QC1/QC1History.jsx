...
const getQCTypeColor = (type) => {
  switch(type) {
    case 'Complete': return 'bg-emerald-100 text-emerald-800 border-emerald-200';
    case 'Partly Clear': return 'bg-amber-100 text-amber-800 border-amber-200';
    default: return 'bg-gray-100 text-gray-800 border-gray-200';
  }
};

const QC1History = ({ orders, onEditClick }) => {
    ...
    return (
      <tr key={order.id || idx} className="hover:bg-amber-50/30 transition-colors border-b border-gray-100 group">
        ...
        <td className="px-4 py-3 text-center whitespace-nowrap text-xs">
          <span className={`px-2 py-0.5 rounded font-bold border ${getQCStatusColor(order.status3)}`}>
            {order.status3 || '-'}
          </span>
        </td>
        <td className="px-4 py-3 text-center whitespace-nowrap text-xs">
          <span className={`px-2 py-0.5 rounded font-bold border ${getQCTypeColor(order.qc1Type)}`}>
            {order.qc1Type || '-'}
          </span>
        </td>
        <td className="px-4 py-3 text-center whitespace-nowrap text-xs text-gray-600 truncate max-w-[150px]" title={order.qcRemarks || '-'}>
          {order.qcRemarks || '-'}
        </td>
        <td className="px-4 py-3 text-center text-xs text-gray-600 whitespace-nowrap">
          {formatTargetDate(order.plannedDates?.['QC1'], 'QC1')}
        </td>
        <td className="px-4 py-3 text-center text-xs text-gray-600 whitespace-nowrap">
    ...
    return (
      <div key={order.id || idx} className="bg-white rounded-xl border border-amber-50 shadow-sm p-4 space-y-3 transition-all hover:shadow-md hover:border-amber-100">
        <div className="flex justify-between items-center pb-2 border-b border-slate-50">
          ...
          <span className={`px-2 py-0.5 rounded text-[8px] font-bold border ${getQCStatusColor(order.status3)}`}>
            {order.status3 || '-'}
          </span>
        </div>
        <div className="grid grid-cols-2 gap-2 text-[11px] bg-slate-50 rounded-lg p-2 border border-slate-100/50">
          ...
          <div className="col-span-2">
            ...
            <span className="text-gray-700">{order.qcRemarks || '-'}</span>
          </div>
          <div>
