...
const MeenaInhouseHistory = ({ orders, onEditClick }) => {
      ...
      <tr key={order.id || idx} className="hover:bg-amber-50/30 transition-colors border-b border-gray-100 group">
        <td className="px-2 py-3 sticky left-0 bg-white group-hover:bg-amber-50 z-10 shadow-[1px_0_0_#e5e7eb] text-center whitespace-nowrap w-32 min-w-[128px]">
          <button
            ...
            title="Edit Meena Inhouse Details"
          >
            <Edit size={12} />
            <span>Edit</span>
          </button>
        ...
        <td className="px-4 py-3 text-center whitespace-nowrap text-xs">
          <span className="px-2 py-0.5 rounded font-bold border bg-emerald-100 text-emerald-800 border-emerald-200">
            {order.meenaInhouseType || order.meenaInhouseStatus || '-'}
          </span>
        </td>
        <td className="px-4 py-3 text-center whitespace-nowrap text-xs font-bold text-gray-900">
          {order.inhouseChillaiWeight ? `${order.inhouseChillaiWeight} g` : '-'}
        </td>
        <td className="px-4 py-3 text-center whitespace-nowrap text-xs font-bold text-gray-900">
          {order.inhouseAfterMeenaPolish ? `${order.inhouseAfterMeenaPolish} g` : '-'}
        </td>
        <td className="px-4 py-3 text-center text-xs text-gray-600 whitespace-nowrap">
          {formatTargetDate(order.plannedDates?.['Meena Inhouse'], 'Meena Inhouse')}
        </td>
        <td className="px-4 py-3 text-center text-xs text-gray-600 whitespace-nowrap">
          {formatDateTime(order.meenaInhouseTimestamp)}
        </td>
        <td className="px-4 py-3 text-center whitespace-nowrap">
          {(() => { const d = calculateDelay(order.plannedDates?.['Meena Inhouse'], order.meenaInhouseTimestamp); return <span className={`px-2 py-0.5 rounded text-[10px] font-bold border ${d.display === '-' ? 'bg-gray-100 text-gray-500 border-gray-200' : d.isDelayed ? 'bg-red-100 text-red-800 border-red-200' : 'bg-emerald-100 text-emerald-800 border-emerald-200'}`}>{d.display}</span>; })()}
        </td>
      ...
      <div key={order.id || idx} className="bg-white rounded-xl border border-amber-50 shadow-sm p-4 space-y-3 transition-all hover:shadow-md hover:border-amber-100">
        ...
        <div className="grid grid-cols-2 gap-2 text-[11px] bg-slate-50 rounded-lg p-2 border border-slate-100/50">
          ...
          <div>
            ...
            {(() => { const d = calculateDelay(order.plannedDates?.['Meena Inhouse'], order.meenaInhouseTimestamp); return <span className={`font-bold ${d.isDelayed ? 'text-red-600' : d.display === '-' ? 'text-gray-400' : 'text-emerald-600'}`}>{d.display}</span>; })()}
          </div>
          <div>
          ...
          <div>
            ...
            <span className="text-gray-700 font-semibold">{order.inhouseChillaiWeight ? `${order.inhouseChillaiWeight} g` : '-'}</span>
          </div>
          <div>
            <span className="text-gray-400 block uppercase text-[8px] tracking-tight">After Polish</span>
            <span className="text-gray-700 font-semibold">{order.inhouseAfterMeenaPolish ? `${order.inhouseAfterMeenaPolish} g` : '-'}</span>
          </div>
        </div>
        <div className="pt-2 border-t border-slate-100 mt-1">
          ...
          >
            <Edit size={12} /> Edit Meena Inhouse
          </button>
        </div>
      </div>
    );
...
export default MeenaInhouseHistory;
