...
const getQC3StatusColor = (status) => {
  switch(status) {
    case 'QC Ok': case 'QC Okay': return 'bg-emerald-100 text-emerald-800 border-emerald-200';
    case 'QC Reject': return 'bg-red-100 text-red-800 border-red-200';
    default: return 'bg-gray-100 text-gray-800 border-gray-200';
  }
};

const QC3History = ({ orders, onEditClick }) => {
    ...
    return (
      <tr key={order.id || idx} className="hover:bg-amber-50/30 transition-colors border-b border-gray-100 group">
        ...
        <td className="px-4 py-3 text-center whitespace-nowrap text-xs">
          {order.qc3Status ? (
            <span className={`px-2 py-0.5 rounded font-bold border ${getQC3StatusColor(order.qc3Status)}`}>
              {order.qc3Status}
            </span>
        ...
        <td className="px-4 py-3 text-center whitespace-nowrap">
          {(() => { const d = calculateDelay(order.plannedDates?.['QC3'], order.qc3Timestamp); return <span className={`px-2 py-0.5 rounded text-[10px] font-bold border ${d.display === '-' ? 'bg-gray-100 text-gray-500 border-gray-200' : d.isDelayed ? 'bg-red-100 text-red-800 border-red-200' : 'bg-emerald-100 text-emerald-800 border-emerald-200'}`}>{d.display}</span>; })()}
        </td>
    ...
    return (
      <div key={order.id || idx} className="bg-white rounded-xl border border-amber-50 shadow-sm p-4 space-y-3 transition-all hover:shadow-md hover:border-amber-100">
        ...
        <div className="grid grid-cols-2 gap-2 text-[11px] bg-slate-50 rounded-lg p-2 border border-slate-100/50">
          ...
          <div>
            <span className="text-gray-400 block uppercase text-[8px] tracking-tight">QC3 Status</span>
            {order.qc3Status ? (
              <span className={`px-1.5 py-0.5 rounded text-[8px] font-bold border inline-block ${getQC3StatusColor(order.qc3Status)}`}>{order.qc3Status}</span>
            ) : <span className="text-gray-400">-</span>}
          </div>
          <div>
          ...
          <div>
            ...
            {(() => { const d = calculateDelay(order.plannedDates?.['QC3'], order.qc3Timestamp); return <span className={`font-bold ${d.isDelayed ? 'text-red-600' : d.display === '-' ? 'text-gray-400' : 'text-emerald-600'}`}>{d.display}</span>; })()}
          </div>
          <div>
        ...
        {order.qc3Remarks && (
          <div className="text-[10px] text-gray-500 border-t border-slate-100 pt-1.5 truncate">
            <strong>Remarks: </strong>{order.qc3Remarks}
          </div>
        )}
