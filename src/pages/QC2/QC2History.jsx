...
const getQCStatusColor = (status) => {
  switch(status) {
    case 'QC Okay': return 'bg-emerald-100 text-emerald-800 border-emerald-200';
    case 'QC Reject': return 'bg-red-100 text-red-800 border-red-200';
    default: return 'bg-gray-100 text-gray-800 border-gray-200';
  }
};

const QC2History = ({ orders, onEditClick }) => {
  ...
  const tableHeaders = [
    ...
    { label: 'QC2 Status', className: 'text-center' },
    { label: 'QC2 Remarks', className: 'text-center' },
  ...
  const renderRow = (order, idx) => {
    ...
    const qc2Stat = order.qc2Status || order.status12;
    return (
    ...
    return (
      <div key={order.id || idx} className="bg-white rounded-xl border border-amber-50 shadow-sm p-4 space-y-3 transition-all hover:shadow-md hover:border-amber-100">
        ...
        <div className="grid grid-cols-2 gap-2 text-[11px] bg-slate-50 rounded-lg p-2 border border-slate-100/50">
          ...
          <div>
            <span className="text-gray-400 block uppercase text-[8px] tracking-tight">QC2 Status</span>
            {qc2Stat ? (
              <span className={`px-1.5 py-0.5 rounded text-[8px] font-bold border inline-block ${getQCStatusColor(qc2Stat)}`}>{qc2Stat}</span>
            ) : <span className="text-gray-400">-</span>}
          </div>
          <div>
          ...
          <div>
            ...
            {(() => { const d = calculateDelay(order.plannedDates?.['QC2'], order.qc2Timestamp); return <span className={`font-bold ${d.isDelayed ? 'text-red-600' : d.display === '-' ? 'text-gray-400' : 'text-emerald-600'}`}>{d.display}</span>; })()}
          </div>
          <div>
        ...
        {order.qc2Remarks && (
          <div className="text-[10px] text-gray-500 border-t border-slate-100 pt-1.5 truncate">
            <strong>QC2 Remarks: </strong>{order.qc2Remarks}
          </div>
        )}
