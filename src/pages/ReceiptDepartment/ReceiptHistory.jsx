...
const getReceiptTypeColor = (type) => {
  return type === 'By Hand' ? 'bg-amber-100 text-amber-800 border-amber-200' : 'bg-blue-100 text-blue-800 border-blue-200';
};

const ReceiptHistory = ({ orders, onEditClick }) => {
    ...
    return (
      <tr key={order.id || idx} className="hover:bg-amber-50/30 transition-colors border-b border-gray-100 group">
        ...
        <td className="px-4 py-3 text-center whitespace-nowrap text-xs">
          {order.receiptType ? (
            <span className={`px-2 py-0.5 rounded font-bold border ${getReceiptTypeColor(order.receiptType)}`}>
              {order.receiptType}
            </span>
          ) : <span className="text-gray-400">-</span>}
        </td>
        <td className="px-4 py-3 text-center text-xs text-gray-600 whitespace-nowrap">
          {order.receiptPersonCourier || '-'}
        </td>
        <td className="px-4 py-3 text-center text-xs font-bold text-gray-900 whitespace-nowrap">
          {order.receiptGrossWeight ? `${order.receiptGrossWeight} g` : '-'}
        </td>
        <td className="px-4 py-3 text-center text-xs text-gray-600 whitespace-nowrap max-w-[150px] truncate" title={order.receiptRemarks}>
          {order.receiptRemarks || '-'}
        </td>
        <td className="px-4 py-3 text-center text-xs text-gray-600 whitespace-nowrap">
          {formatTargetDate(order.plannedDates?.['Receipt'], 'Receipt')}
        </td>
        <td className="px-4 py-3 text-center text-xs text-gray-600 whitespace-nowrap">
          {formatDateTime(order.receiptTimestamp)}
        </td>
        <td className="px-4 py-3 text-center whitespace-nowrap">
          {(() => { const d = calculateDelay(order.plannedDates?.['Receipt'], order.receiptTimestamp); return <span className={`px-2 py-0.5 rounded text-[10px] font-bold border ${d.display === '-' ? 'bg-gray-100 text-gray-500 border-gray-200' : d.isDelayed ? 'bg-red-100 text-red-800 border-red-200' : 'bg-emerald-100 text-emerald-800 border-emerald-200'}`}>{d.display}</span>; })()}
        </td>
    ...
    return (
      <div key={order.id || idx} className="bg-white rounded-xl border border-amber-50 shadow-sm p-4 space-y-3 transition-all hover:shadow-md hover:border-amber-100">
        ...
        <div className="grid grid-cols-2 gap-2 text-[11px] bg-slate-50 rounded-lg p-2 border border-slate-100/50">
          ...
          <div>
            <span className="text-gray-400 block uppercase text-[8px] tracking-tight">Receipt Type</span>
            {order.receiptType ? (
              <span className={`px-1.5 py-0.5 rounded text-[8px] font-bold border inline-block ${getReceiptTypeColor(order.receiptType)}`}>{order.receiptType}</span>
            ) : <span className="text-gray-400">-</span>}
          </div>
          <div>
            <span className="text-gray-400 block uppercase text-[8px] tracking-tight">Person/Courier</span>
            <span className="text-gray-700 font-semibold">{order.receiptPersonCourier || '-'}</span>
          </div>
          <div>
            <span className="text-gray-400 block uppercase text-[8px] tracking-tight">Gross Weight</span>
          ...
          <div>
            ...
            {(() => { const d = calculateDelay(order.plannedDates?.['Receipt'], order.receiptTimestamp); return <span className={`font-bold ${d.isDelayed ? 'text-red-600' : d.display === '-' ? 'text-gray-400' : 'text-emerald-600'}`}>{d.display}</span>; })()}
          </div>
        </div>
        {order.receiptRemarks && (
          <div className="text-[10px] text-gray-500 border-t border-slate-100 pt-1.5 truncate">
            <strong>Remarks: </strong>{order.receiptRemarks}
          </div>
        )}
        <div className="pt-2 border-t border-slate-100 mt-1">
          ...
          >
            <Edit size={12} /> Edit Receipt Details
          </button>
        </div>
      </div>
    );
