...
export const HuidLabel = () => {
  ...
  const basePendingOrders = useMemo(() => {
    return orders.filter(o => 
      ...
      !( (o.huidStatus === 'Huid Complete' || o.huidStatus === 'No Huid' || o.huidStatus === 'No HUID') && o.labelingStatus === 'Yes' )
    );
  }, [orders]);

  const baseHistoryOrders = useMemo(() => {
    return orders.filter(o => !!o.huidStatus);
  }, [orders]);
  ...
  // Split orders based on Huid/Label stage
  // 1. Pending: Completed in QC3 stage (qc3Status is 'QC Ok' or 'QC Okay' or status15 is 'Complete') AND HUID status is not set
  

  // 2. History: processed HUID orders with status 'Huid Complete', 'Sent In Huid' or 'No Huid'
