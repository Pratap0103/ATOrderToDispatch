...
const getHuidStatusColor = (status) => {
  switch(status) {
    case 'Huid Complete': return 'bg-emerald-100 text-emerald-800 border-emerald-200';
    case 'Sent In Huid': return 'bg-blue-100 text-blue-800 border-blue-200';
    case 'No Huid': return 'bg-gray-100 text-gray-800 border-gray-200';
    default: return 'bg-gray-100 text-gray-800 border-gray-200';
  }
};

const LabelHistory = ({ orders, onEditClick }) => {
  ...
  const tableHeaders = [
    ...
    { label: 'Huid Status', className: 'text-center' },
    { label: 'Labeling Status', className: 'text-center' },
    ...
    return (
      <tr key={order.id || idx} className="hover:bg-amber-50/30 transition-colors border-b border-gray-100 group">
        ...
        <td className="px-4 py-3 text-center whitespace-nowrap text-xs">
          {order.huidStatus ? (
            <span className={`px-2 py-0.5 rounded font-bold border ${getHuidStatusColor(order.huidStatus)}`}>
              {order.huidStatus}
            </span>
          ) : <span className="text-gray-400">-</span>}
        </td>
        <td className="px-4 py-3 text-center text-xs font-semibold whitespace-nowrap">
          {order.labelingStatus ? (
            <span className={`px-2 py-0.5 rounded font-bold border ${order.labelingStatus === 'Yes' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-red-50 text-red-700 border-red-200'}`}>
              {order.labelingStatus}
            </span>
          ) : '-'}
        </td>
        <td className="px-4 py-3 text-center text-xs text-gray-600 whitespace-nowrap font-medium">
          {order.sentCompanyName || '-'}
        </td>
        <td className="px-4 py-3 text-center text-xs font-bold text-gray-900 whitespace-nowrap">
          {order.sentHuidLabelPcs !== undefined && order.sentHuidLabelPcs !== null ? `${order.sentHuidLabelPcs} pcs` : '-'}
        </td>
        <td className="px-4 py-3 text-center text-xs text-gray-600 whitespace-nowrap max-w-[150px] truncate" title={order.huidRemarks}>
          {order.huidRemarks || '-'}
        </td>
        <td className="px-4 py-3 text-center text-xs text-gray-600 whitespace-nowrap">
          {formatTargetDate(order.plannedDates?.['Label'], 'Label')}
        </td>
        <td className="px-4 py-3 text-center text-xs text-gray-600 whitespace-nowrap">
          {formatDateTime(order.huidTimestamp)}
        </td>
        <td className="px-4 py-3 text-center whitespace-nowrap">
          {(() => { const d = calculateDelay(order.plannedDates?.['Label'], order.huidTimestamp); return <span className={`px-2 py-0.5 rounded text-[10px] font-bold border ${d.display === '-' ? 'bg-gray-100 text-gray-500 border-gray-200' : d.isDelayed ? 'bg-red-100 text-red-800 border-red-200' : 'bg-emerald-100 text-emerald-800 border-emerald-200'}`}>{d.display}</span>; })()}
        </td>
      ...
      <div key={order.id || idx} className="bg-white rounded-xl border border-amber-50 shadow-sm p-4 space-y-3 transition-all hover:shadow-md hover:border-amber-100">
        ...
        <div className="grid grid-cols-2 gap-2 text-[11px] bg-slate-50 rounded-lg p-2 border border-slate-100/50">
          ...
          <div className="text-center">
            ...
            {order.huidStatus ? (
              <span className={`px-1.5 py-0.5 rounded text-[8px] font-bold border inline-block ${getHuidStatusColor(order.huidStatus)}`}>{order.huidStatus}</span>
            ) : <span className="text-gray-400">-</span>}
          </div>
          <div className="text-center">
            <span className="text-gray-400 block uppercase text-[8px] tracking-tight">Labeling</span>
            {order.labelingStatus ? (
              <span className={`px-1.5 py-0.5 rounded text-[8px] font-bold border inline-block ${order.labelingStatus === 'Yes' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-red-50 text-red-700 border-red-200'}`}>{order.labelingStatus}</span>
            ) : <span className="text-gray-400">-</span>}
          </div>
          <div className="text-center">
            <span className="text-gray-400 block uppercase text-[8px] tracking-tight">Company</span>
            <span className="text-gray-700 font-semibold">{order.sentCompanyName || '-'}</span>
          </div>
          <div className="text-center">
            <span className="text-gray-400 block uppercase text-[8px] tracking-tight">Sent Pcs</span>
          ...
          <div className="text-center">
            ...
            {(() => { const d = calculateDelay(order.plannedDates?.['Label'], order.huidTimestamp); return <span className={`font-bold ${d.isDelayed ? 'text-red-600' : d.display === '-' ? 'text-gray-400' : 'text-emerald-600'}`}>{d.display}</span>; })()}
          </div>
        </div>
        {order.huidRemarks && (
          <div className="text-[10px] text-gray-500 border-t border-slate-100 pt-1.5 truncate text-center">
            <strong>Remarks: </strong>{order.huidRemarks}
          </div>
        )}
