...
const OrderFormEdit = ({ isOpen, onClose, onSave, order }) => {
  ...
  useEffect(() => {
    if (isOpen && order) {
      // Backward-compat: migrate legacy single `image` field → `images` array
      const migratedOrder = { ...order };
      if (!migratedOrder.images || !Array.isArray(migratedOrder.images)) {
        migratedOrder.images = migratedOrder.image ? [migratedOrder.image] : [];
      }
      delete migratedOrder.image;
      migratedOrder.orderStage = migratedOrder.manualOrderStage || migratedOrder.orderStage;
  ...
  const handleSubmit = (e) => {
    ...
    onSave({ ...formData, manualOrderStage: formData.orderStage });
    onClose();
  };
