...
import ReceiveInStockPending from './ReceiveInStockPending';
...
export const ReceivedInStock = () => {
  ...
  const baseHistoryOrders = useMemo(() => {
    return orders.filter(o => 
      o.receiveInStockStatus === 'Received'
    );
  }, [orders]);
  ...
  // Split orders based on Stock ingestion stage
  // 1. Pending: Complete in Huid/Label (huidStatus is set or status15 is 'Complete') AND not yet received in stock
  

  // 2. History: processed Stock receipt where receiveInStockStatus is 'Received'
  ...
  return (
    <div className="p-0 sm:p-2 md:p-6 space-y-2 md:space-y-6 flex flex-col h-full min-h-0">
      ...
      <ReceiveInStockForm
        ...
        onSave={handleSaveReceiveInStock}
        order={selectedOrder}
      />
...
export default ReceivedInStock;
